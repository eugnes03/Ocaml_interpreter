...
let rec desugar_expr (e : expr) : lexpr =
      ...
      | Num n -> Num n
      | Bool b -> Bool b
      | Var id -> Var (String.uppercase_ascii id)  
...
let rec translate (e : lexpr) : stack_prog =
  ...
  | Fun("_", body) -> [Fun("C", [Swap; Assign "C"] @ translate body @ [Swap; Return])]
  | Fun (name, body) -> [Fun(capitalize_string name, [Swap; Assign name] @ translate body @ [Swap; Return])]
  | Num n -> [Push (Num n)]
  | Bool b -> [Push (Bool b)]
  | Unit -> [Push Unit]
  | Var id -> [Lookup id]
  ...
  | Uop (u1, u2) -> 
    (match u1 with
    | Neg -> translate u2 @ [Push (Num 0); Add; Push (Num 0); Sub]
    | Not -> translate u2 @ [If ([Push (Bool false)], [Push (Bool true)])])
  | Bop (b1, b2, b3) ->
    (match b1 with
    | Add -> translate b3 @ [Push (Num 0); Add] @ translate b2 @ [Add]
    | Sub -> translate b3 @ [Push (Num 0); Add] @ translate b2 @ [Sub]
    | Mul -> translate b3 @ [Push (Num 0); Add] @ translate b2 @ [Mul]
    | Div -> translate b3 @ [Push (Num 0); Add] @ translate b2 @ [Div]
    | Lt -> translate b3 @ [Push (Num 0); Add] @ translate b2 @ [Lt]
    | And -> translate b2 @ [If (translate b3,[Push (Bool false)])]
    | Or -> translate b2 @ [If ([Push (Bool true)], translate b3)]
    | Gt -> translate b2 @ [Push (Num 0); Add] @ translate b3 @ [Lt]
    | Lte -> translate (Uop (Not, Bop (Gt, b2, b3)))
    | Gte -> translate (Uop (Not, Bop (Lt, b2, b3)))
    | Eq -> translate (Bop (And, (Uop (Not, Bop (Gt, b2, b3))), (Uop (Not, Bop (Lt, b2, b3)))))
    | Neq -> translate (Uop (Not, (Bop (Eq, b2, b3))))) 
  | Ife(b, l, r) -> translate b @ [If (translate l, translate r)]
